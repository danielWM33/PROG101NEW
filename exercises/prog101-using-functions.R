# Fill in the blanks so the answers are all 2. Use help for unfamiliar functions
# if necessary.
log10(100)
sqrt(4)
ceiling(2.0)
?ceiling

# Find the functions for cosine and sine. What are the cosine and sine of 0?
?sine
?sin
sin(0)
cos(0)

# What do you think the following functions do? Just go off the name, don't look
# them up.
read.csv()
#reads every data points and spits it out formatted? not math related
boxplot()
#Creates data that can be easily used in Excel to create boxplot
startsWith()
#Filters out any datapoint that does not start with
